...
10. **Cloudflare Access — protect the dashboard:**
    ...
    - **(Optional) Personalize the login page.** Out of the box the page shows generic Cloudflare Access chrome on your auth subdomain. To replace it with your own logo, colors, and copy: *Zero Trust → Reusable components → Custom pages → Access login page → Manage*. Fields: organization name, logo URL (HTTPS — a raw GitHub URL, R2, any public host), header text, footer text, background color, text color. Scriptable via `PUT /accounts/{account_id}/access/organizations` (field `login_design`) if you'd rather keep the config in source. **Gotcha:** `text_color` applies to text rendered *on* the white OTP card (Email label, header, footer) — it needs to contrast white, not your page background. Picking a dark bg and dark text is usually what you want.
